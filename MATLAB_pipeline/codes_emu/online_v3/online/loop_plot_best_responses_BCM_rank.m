function loop_plot_best_responses_BCM_rank(chaux, muonly, rankfirst, ranklast_def, ...
                                           effect_rows_2, win_cent, nstd, phase, ...
                                           concat_folder, varargin)% Plots the best max_stims responses of a channel ch and cluster i and IFR of all responses.
ipr = inputParser;
...
grapes_path = fullfile(concat_folder, grapes_name);
if ~exist(grapes_path, 'file')
    grapes_path = fullfile(output_dir, grapes_name);  % fallback to output dir
end
if ~exist(grapes_path, 'file')
    grapes_path = grapes_name;  % last resort: pwd (old flat layout)
end
grapes = load(grapes_path);
