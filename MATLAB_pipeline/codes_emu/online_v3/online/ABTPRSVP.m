...
addParameter(ipr, 'use_background_collection', true);  % Use timer-based background data collection
addParameter(ipr, 'collection_poll_interval', 0.050);  % Poll interval for background collection (seconds)
...
if params.is_online
    ...
    % Initialize BackgroundDataCollector for timer-based buffer collection
    % Uses MATLAB timer on main thread - can access device_com
    use_background_collection = params.use_background_collection;
    if use_background_collection
        bg_collector = BackgroundDataCollector(numel(channels), ...
            'sample_rate', 30000, ...
            'use_photodiode', params.use_photodiodo, ...
            'poll_interval', params.collection_poll_interval, ...
            'max_duration', 60);  % 60 sec max per trial
        
        fprintf('\n=== BACKGROUND DATA COLLECTION ENABLED ===\n');
        fprintf('Timer-based collection runs on main thread.\n');
...
try
        ...
        for irep = 1:Nseq
            ...
            if params.is_online
                ...
                if use_background_collection && ~isempty(bg_collector)
                    % Reset and start timer-based collection for this trial
                    bg_collector.reset_buffers();
                    bg_collector.start();
            ...
            if params.is_online
                % === GET TRIAL DATA ===
                if use_background_collection && ~isempty(bg_collector)
                    % Stop background collection and get accumulated data
                    bg_collector.stop();
                    [data, trial_datacounter, events] = bg_collector.get_trial_data(true);
                    ...
                    %if n_scr == 1 && irep == 1
                    %    stats = bg_collector.get_statistics();
                    %    fprintf('\n--- Continuous collection stats (trial 1) ---\n');
                    %    fprintf('  Collections: %d, Samples: %d\n', stats.collection_count, stats.total_samples);
                    %    fprintf('  Buffer usage: %.1f%%\n', stats.buffer_usage);
                    %    fprintf('----------------------------------------------\n');
                    %end
                else
    ...
    if params.is_online && params.disable_interactions
        device_com('close');
    end
    ...
    % Cleanup background data collector on error
    if exist('bg_collector', 'var') && ~isempty(bg_collector)
        try
            bg_collector.stop();
            delete(bg_collector);
...
try
    if params.is_online && n_scr > 0
        ...
        % Close device (only when disable_interactions - otherwise recording system handles it)
        if params.is_online && params.disable_interactions
            device_com('close');
        end
...
% Cleanup background data collector
if exist('bg_collector', 'var') && ~isempty(bg_collector)
    try
        bg_collector.stop();
    catch
    ...
    if params.is_online
        ...
        fprintf('Timer-based data collection completed successfully.\n');
        fprintf('=================================================\n');
    end
    delete(bg_collector);
end
