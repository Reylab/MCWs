...
%     
    ...
    if showwins
        ...
        % Build face tracking info struct for select_stims
        face_tracking = struct();
        face_tracking.enabled = ~isempty(pics_lookup_table) && height(pics_lookup_table) > 0;
        face_tracking.lookup_table = pics_lookup_table;
        face_tracking.ImageNames = experiment.ImageNames;
        face_tracking.stim_numbers = data2plot.stim_number;
        face_tracking.current_faces = current_faces_count;
        face_tracking.current_nonfaces = current_nonfaces_count;
        
        sel_ix = select_stims(fc, n_scr, true, lbl, show_sel_count, miniscr_pics_count, true, face_tracking);
