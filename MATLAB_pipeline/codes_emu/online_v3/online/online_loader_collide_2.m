% filepath: /home/user/Documents/GitHub/codes_emu/tasks/online_v3/online/online_loader_collide_2.m
...
% Add paths early so safe_load_mat and other functions are available
addpath(fullfile(fileparts(mfilename('fullpath')),'../../..')) % always / for this trick
custompath = reylab_custompath({'useful_functions','useful_functions/tasks_tools','wave_clus_reylab','wave_split','codes_for_analysis','mex','tasks/online_v3/online'});
...
% Always show dialog to verify/edit priority channels (even if no templates/preprocessing)
...
% Load experiment with locking (no ready signal wait needed here)
[loaded_data, load_success] = safe_load_mat(exp_prop_file, 'experiment');
if load_success
    experiment = loaded_data.experiment;
else
    ...
    while ~exper_finished && ~strcmp(experiment.msgs_Mat.exper_aborted,msg_received)
        ...
        if b_use_blanks
            grapes = update_grapes_blanks(grapes, pics_onset, ...
                ...
                chan_label, 1, [], ...
                scr_config.pics2use, scr_id, b_circshiftblanks,params.is_online);
        else
            grapes = update_grapes(grapes,pics_onset,stimulus,wc_sp_index_cell{scr_id},channels,chan_label,1,[],scr_config.pics2use,scr_id,params.is_online);
        end
            ...
            while numel(done_chs_ix) < numel(channels)
                ...
                if b_use_blanks
                    grapes = update_grapes_blanks(grapes, pics_onset, ...
                                                  ...
                                                  scr_id, b_circshiftblanks, params.is_online);
                else
                    grapes = update_grapes(grapes,pics_onset,stimulus, ...
                                           wc_sp_index_cell{scr_id}(ch_ix_out), ...
                                           channels(ch_ix_out),chan_label(ch_ix_out), ...
                                           false,classes_out, scr_config.pics2use,scr_id,params.is_online);
                end
        ...
        subscr_exp_prop_file_name = sprintf('exp_prop_subscr_%d.mat', scr_id);
