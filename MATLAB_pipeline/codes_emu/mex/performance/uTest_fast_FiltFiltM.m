function Copy_of_uTest_FiltFiltM(doSpeed)
% Unit test: FiltFiltM
% This is a routine for automatic testing. It is not needed for processing and
% can be deleted or moved to a folder, where it does not bother.
%
% uTest_fast_filtfilt(doSpeed)
% INPUT:
...
% X = rand(1000, 1);
% Y = fast_filtfilt(B, A, X);
% Z = fast_filtfilt(B, A, reshape(X, 1, 1, []), []);
...
% fprintf('\nProvoke problems:\n');
% try
%    Y = fast_filtfilt(B, A, rand(4));  %#ok<NASGU>
%    tooLazy = true;
% catch
...
% 
% try
%    Y = fast_filtfilt(B, A, rand(4), 2);  %#ok<NASGU>
%    tooLazy = true;
% catch
...
% 
% try
%    Y = fast_filtfilt(B, A, rand(1, 100), 3);  %#ok<NASGU>
%    tooLazy = true;
% catch
...
% 
% try
%    Y = fast_filtfilt(B, A, rand(1, 100), -3.1);  %#ok<NASGU>
%    tooLazy = true;
% catch
...
for iTest = 1:length(TestSize)
   ...
   tic;
   for i = 1:nLoop
      newy = fast_filtfilt(B, A, x);  %#ok<NASGU>
      clear('newy');
   end
   elapsed1 = toc;
   ...
   newy  = fast_filtfilt(B, A, x);
   if isequal(origy, newy)
      fprintf('    Results are identical\n\n');
   elseif all(abs(origy(:) - newy(:)) < 50 * eps)
