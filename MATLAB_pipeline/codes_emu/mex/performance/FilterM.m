...
%   See FilterX_fc.c for instructions for a manual compilation.
%
% EXAMPLES:
...
% Call the calculator: ---------------------------------------------------------
if replyZ
   [Y, Z] = FilterX_fc(b, a, X, Z, Reverse);
else
   Y = FilterX_fc(b, a, X, Z, Reverse);
end
...
if isempty(which(fullfile(FuncPath, 'FilterX_fc.c')))
   error(['JSimon:', FuncName, ':Compile'], ...
      ['*** ', FuncName, ': Cannot find the C-file "FilterX_fc.c" ???'])
end
...
if isunix
   ...
   Flags = {'-O', 'CFLAGS="\$CFLAGS','-std=c99"','-compatibleArrayDims' ,'FilterX_fc.c'};
else
   Flags = {'-O','-compatibleArrayDims', 'FilterX_fc.c'};
end
%what about -compatibleArrayDims
