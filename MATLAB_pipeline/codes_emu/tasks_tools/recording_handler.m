    ...
    methods
        function obj=recording_handler(params, fname) %prepare recording
            ...
            if strcmp(params.system,'RIP')
                % Skip xippmex connection if already opened by device_com.
                % Probing xippmex state when uninitialized either crashes
                % MATLAB or corrupts MEX internal state. The caller must
                % set params.xippmex_connected = true when device_com has
                % already established the connection (e.g. ABTPRSVPTEST).
                if ~isfield(params, 'xippmex_connected') || ~params.xippmex_connected
                    try
                        xippmex('tcp');
                    catch
                        xippmex('close');
