classdef online_sorter < handle
    ...
    methods
        ...
        function update_max_workers(obj, new_maxworkers)
            % Dynamically update the maximum number of workers
            %
            % Input:
            %   new_maxworkers - new maximum worker count
            
            if new_maxworkers < 1
                warning('Cannot set maxworkers below 1');
                return;
            end
            
            % Wait for any running futures before resizing
            running_futures = find(arrayfun(@(x) x.ID ~= -1, obj.futures));
            if ~isempty(running_futures)
                fprintf('Waiting for %d running sorting jobs before resizing...\n', numel(running_futures));
                for i = running_futures
                    wait(obj.futures(i));
                end
                obj.retrieve_sorting_results();
            end
            
            old_maxworkers = obj.maxworkers;
            obj.maxworkers = new_maxworkers;
            
            % Resize futures array
            obj.futures = parallel.FevalFuture;
            obj.futures(1:obj.maxworkers) = parallel.FevalFuture;
            obj.futures_chs = cell(obj.maxworkers, 1);
            
            fprintf('online_sorter: updated maxworkers from %d to %d\n', old_maxworkers, new_maxworkers);
        end
        
        function n = get_active_workers(obj)
            % Get number of currently active sorting workers
            n = sum(arrayfun(@(x) x.ID ~= -1, obj.futures));
        end
