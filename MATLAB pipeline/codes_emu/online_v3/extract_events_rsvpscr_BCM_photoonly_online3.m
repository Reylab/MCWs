...
for i = 1:Nscr
    ...
    stimulus{i}(cellfun(@isempty,{stimulus{i}.terna_onset}))=[];
...
for i = 1:n_scr_ended
    ...
    plot(tdiff(:)-1000*ISI,'b'); %should be 0 ms
    xlim([0 seq_length(i)*Nseq(i)+1])
    h_legend = legend(sprintf('stimulus "error" (ms). %2.0f%% lower than 2ms',100*sum(abs(tdiff(:)-1000*ISI)<2)/numel(tdiff)),'location','best');legend('boxoff')
end
