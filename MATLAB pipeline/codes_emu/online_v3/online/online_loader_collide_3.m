function online_loader_collide_3()
...
nwins_best_stims = 12; % No of windows for best_stims
...
if params.run_num == 1 && ...
   (strcmp(experiment.subtask, 'DynamicScr') || ...
    strcmp(experiment.subtask, 'DynamicSeman') || ...
    strcmp(experiment.subtask, 'CategLocaliz'))
...
%priority_chs_ranking = [257:273 289:296 330:337 353:360 377:384];
...
%% Read priority_chs_ranking from .map file
...
%% Find latest DynamicScr folder for fallback
latest_dynscr_folder = find_latest_dynamicscr_folder(params.root_processing, params.sub_ID);

%% Template loading with fallback to latest DynamicScr results
...
elseif params.templates_required
    ...
    elseif isfile(TEMPLATES_MS)
        ...
        disp('TEMPLATES_WC not found, using TEMPLATES_MS from rec_metadata.')
    elseif ~isempty(latest_dynscr_folder)
        % Fallback: look in latest DynamicScr results folder
        fallback_templates_wc = fullfile(latest_dynscr_folder, 'results', 'templates_wc_offline.mat');
        fallback_templates_ms = fullfile(latest_dynscr_folder, 'results', 'templates_ms.mat');
        if isfile(fallback_templates_wc)
            TEMPLATES_FILE = fallback_templates_wc;
            templates_source = sprintf('latest DynamicScr results: %s', latest_dynscr_folder);
            templates_loaded_from_fallback = true;
            disp(['Templates found in latest DynamicScr: ' fallback_templates_wc])
        elseif isfile(fallback_templates_ms)
            TEMPLATES_FILE = fallback_templates_ms;
            templates_source = sprintf('latest DynamicScr results: %s', latest_dynscr_folder);
            templates_loaded_from_fallback = true;
            disp(['Templates found in latest DynamicScr: ' fallback_templates_ms])
        else
            error('Templates required but not found in rec_metadata or latest DynamicScr folder')
        end
    else
        error('Templates required but not found (no previous DynamicScr folder found)')
    end
    % templates_source will be shown in combined dialog later
end
...
%% Notches/preprocessing loading with fallback to latest DynamicScr results
if par.preprocessing
    ...
    elseif ~isempty(latest_dynscr_folder)
        % Fallback: look in latest DynamicScr results folder
        fallback_preprocessing = fullfile(latest_dynscr_folder, 'results', 'pre_processing_info.mat');
        if isfile(fallback_preprocessing)
            load(fallback_preprocessing,'process_info')
            preprocessing_source = sprintf('latest DynamicScr results: %s', latest_dynscr_folder);
            preprocessing_loaded_from_fallback = true;
            disp(['Preprocessing info found in latest DynamicScr: ' fallback_preprocessing])
        else
            error('pre_processing_info.mat not found in rec_metadata or latest DynamicScr folder')
        end
    else
        error('pre_processing_info.mat not found (no previous DynamicScr folder found)')
    end
    % preprocessing_source will be shown in combined dialog later
end

%% Show combined configuration dialog
% Prepare sources for dialog
if ~exist('templates_source', 'var')
    templates_source = '';
end
if ~exist('preprocessing_source', 'var')
    preprocessing_source = '';
end

% Always show dialog to verify/edit priority channels
...
fprintf('Priority channels set to: [%s]\n', num2str(priority_chs_ranking));
...
% Load Pics.xlsx for face/non-face classification lookup
pics_xlsx_path = '/home/user/ReyLab/experimental_files/pics/Pics.xlsx';
if exist(pics_xlsx_path, 'file')
    try
        % Try reading with xlsread first (more reliable for some Excel files)
        [~, ~, raw_data] = xlsread(pics_xlsx_path, 'All pics');
        % First row is header
        header_row = raw_data(1, :);
        % Convert header to strings, handling NaN/missing
        header_str = cell(1, size(header_row, 2));
        for hi = 1:numel(header_row)
            if ischar(header_row{hi})
                header_str{hi} = header_row{hi};
            elseif isstring(header_row{hi})
                header_str{hi} = char(header_row{hi});
            else
                header_str{hi} = '';
            end
        end
        name_col_idx = find(strcmpi(header_str, 'Name'), 1);
        folder_col_idx = find(strcmpi(header_str, 'Folder'), 1);
        
        if isempty(name_col_idx) || isempty(folder_col_idx)
            warning('Pics.xlsx missing required columns (Name or Folder). Available columns: %s', strjoin(header_str, ', '));
            ...
            % Convert to lowercase for names and handle any non-string/NaN entries
            for di = 1:numel(name_data)
                if ischar(name_data{di})
                    name_data{di} = lower(name_data{di});
                elseif isstring(name_data{di})
                    name_data{di} = lower(char(name_data{di}));
                else
                    name_data{di} = '';
                end
                if ischar(folder_data{di})
                    % keep as is
                elseif isstring(folder_data{di})
                    folder_data{di} = char(folder_data{di});
                else
                    folder_data{di} = '';
                end
            end
            % Remove empty rows
            valid_rows = ~cellfun(@isempty, name_data) & ~cellfun(@isempty, folder_data);
            name_data = name_data(valid_rows);
            folder_data = folder_data(valid_rows);
            pics_lookup_table = table(name_data, folder_data, 'VariableNames', {'Name', 'Folder'});
            fprintf('Loaded Pics.xlsx with %d entries for face/non-face lookup\n', height(pics_lookup_table));
        end
    catch ME
        warning('Error reading Pics.xlsx: %s. Face/Non-face tracking will be disabled.', ME.message);
        pics_lookup_table = table();
    end
else
    warning('Pics.xlsx not found at %s. Face/Non-face tracking will be disabled.', pics_xlsx_path);
    pics_lookup_table = table();
end

% Freq tag folder paths for copying selected images from final selection
...
% Load experiment with locking (no ready signal wait needed here - file already exists)
[loaded_data, load_success] = safe_load_mat(exp_prop_file, 'experiment');
if load_success
    experiment = loaded_data.experiment;
else
    error('Failed to load experiment from %s', exp_prop_file);
end
...
try
    ...
    while ~exper_finished && ~strcmp(experiment.msgs_Mat.exper_aborted,msg_received)
        ...
        n_scr = n_scr + 1;
        % Wait for ready signal and load scr_config_cell with locking
        if wait_for_file_ready(exp_prop_file, 60)
            [loaded_data, load_success] = safe_load_mat(exp_prop_file, 'scr_config_cell');
            if load_success
                scr_config_cell = loaded_data.scr_config_cell;
            else
                error('Failed to load scr_config_cell from %s', exp_prop_file);
            end
        else
            error('Timeout waiting for scr_config_cell ready signal from %s', exp_prop_file);
        end
        ...
        if DO_SORTING
            ...
            while numel(done_chs_ix) < numel(channels)
                ...
                if b_use_blanks
                    ...
                    grapes = update_grapes(grapes,pics_onset,stimulus,wc_sp_index_cell{n_scr}(ch_ix_out),channels(ch_ix_out),chan_label(ch_ix_out),false,classes_out, scr_config.pics2use,n_scr,params.is_online);
                end
        ...
        if ~params.disable_interactions && ...
           ~strcmp(experiment.msgs_Mat.exper_finished,msg_received) && ...
            ...
            if contains(experiment.subtask, 'DynamicScr') || contains(experiment.subtask, 'CategLocaliz')
                ...
                if n_scr < 3 && ~isempty(selected2explore) && ...
                   numel(selected2notremove) < experiment.NPICS(n_scr+1)
                    ...
                    if numel(same_units) + numel(same_categories) >= num_pics_to_add && ...
        ...
        if n_scr < numel(experiment.NPICS) && ...
            (contains(experiment.subtask, 'DynamicScr') || ...
            ...
            % Already shown but not removed
            others_not_to_be_shown = pics_not_to_be_shown(ismember(pics_not_to_be_shown, extra_stims));  
            ...
            if num_not_to_be_shown > 0
                fprintf("%d pics_not_to_be_shown in next screening. (%d selected2explore, %d same_units, %d same_categories, %d others) ", ...
                    num_not_to_be_shown, numel(selected2explore_not_to_be_shown), numel(same_units_not_to_be_shown), ...
...
%     end
    ...
    % For final selection, start fresh counts (not carrying over from subscreenings)
    [selected2explore_final, s4miniscr_tbl, ~] = stimulus_selection_windows( data_to_plot, grapes, rank_config, ...
                                                        n_scr, ifr_calculator, nwins_best_stims, ...
                                                        lbl, priority_chs_ranking, ...
                                                        experiment, copy2miniscrfolder, ...
                                                        show_sel_count, showwins, true, ...
                                                        pics_lookup_table, 0, 0);
    
    % Copy selected images to freq_tag folders (RespFaces and RespNonFaces)
...
%     end
    ...
    elseif exist('preprocessing_loaded_from_fallback', 'var') && preprocessing_loaded_from_fallback
        % Loaded from DynamicScr fallback - copy to preserve original
        copy_file(fallback_preprocessing, result_folder, temp_folder);
        disp('Copied pre_processing_info from DynamicScr fallback to results')
    end
    if ~isempty(MAPFILE)
    ...
    if exist('TEMPLATES_FILE', 'var')
        if exist('templates_loaded_from_fallback', 'var') && templates_loaded_from_fallback
            % Loaded from DynamicScr fallback - copy to preserve original
            copy_file(TEMPLATES_FILE, result_folder, temp_folder);
            disp('Copied templates from DynamicScr fallback to results')
        else
            % Loaded from rec_metadata - move to results
    ...
    if contains(experiment.subtask,'DynamicSeman') && params.acq_network
