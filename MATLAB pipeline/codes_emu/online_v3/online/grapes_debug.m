...
if ~block_by_block
    ...
    if grapes_offline
        data = sort_responses_table(data, priority_chs_ranking);
    else
        data = sort_responses_table_online(data, priority_chs_ranking);
    end
    ...
    if plot_best_stims
        show_best_stims_wins = true; short_win = false;
        copy2miniscrfolder = false; show_sel_count = true;
        group_stims = true; 
        % best stims plot
        data_to_plot = create_best_stims_table(experiment, grapes, data, nwin, group_stims, priority_chs_ranking);
    ...
    for scr_id=1:length(scr_config_cell)
        ...
        % Check if 'stim_trial_count' exists, add if missing
        if ismember('stim_trial_count', experiment.ImageNames.Properties.VariableNames)
