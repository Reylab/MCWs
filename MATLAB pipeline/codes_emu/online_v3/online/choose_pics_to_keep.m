function [pics_to_keep, pics_removed, categ_localiz_history_updated] = choose_pics_to_keep(experiment, next_pics, ISI, nxt_scr, ...
    ...
    function [pics_to_keep, pics_removed, npics_scr, scr_config] = update_selection()
        ...
        if ~b_last_block && npics_scr < Npics
            ...
            if contains(experiment.subtask, 'CategLocaliz')
                [random_pics, categ_localiz_history_updated] = get_categ_localiz_random(experiment, categ_localiz_history, random_pics_count);
            else
...
function [random_pics, categ_localiz_history] = get_categ_localiz_random(experiment, categ_localiz_history, ...
                                                random_pics_count)
    ...
    for i_stim_cat = 1:numel(categ_localiz_history)
        ...
        stim_cat = categ_localiz_history{i_stim_cat}.category;
        cat_pics = [];
...
%         str_pics_added = "";
        if 0 < numel(cat_pics)
            for su_idx = 1:numel(cat_pics)
                pic_id = find(cellfun(@(x)strcmp(x, sprintf("%s-%d.jpg",lower(stim_cat), ...
                                                 cat_pics(su_idx))), ...
                                                 experiment.ImageNames.name));
%                 experiment.ImageNames(pic_id,:).selectable = select_status;
%                 str_pics_added = sprintf("%s %s(%d)", str_pics_added, experiment.ImageNames(pic_id,:).name{1}, pic_id);
                random_pics = [random_pics; pic_id];
            end
        end
%         fprintf("Random pics for %s (%d): %s \n", stim_cat, numel(cat_pics), str_pics_added);
    end
end