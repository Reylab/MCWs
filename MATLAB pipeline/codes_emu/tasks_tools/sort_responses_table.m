function sorted_datat=sort_responses_table(data,priority_chnums)
...
%         data_ms0];
    ...
    data_nanzscore = data(isnan(data.zscore),:);
    data = data(~isnan(data.zscore),:);
    data_less_than_4_trials = data(data.ntrials<=4,:);
    data = data(data.ntrials>4,:);
    ...
    data_ms1_st0_hzc_gl1_lt = sortrows(data(data.min_spk_test==1 & data.p_test==0 & ~(data.zscore<4.5) & data.good_lat==1 & data.ntrials<=4 ,:),{'zscore','ntrials',},{'descend','descend'});
