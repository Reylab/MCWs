function [data, rank_config] = create_responses_data_parallel_align(grapes,selected_stim,unit_label, ...
    ...
    if numel(channels)>1
        if b_parallel
            ...
            for chi=channels_order
                futures(chi) = parfeval(@get_responses_channel_align, 1, grapes.rasters.(channels{chi}), channels(chi), ...
            ...
            for chi = 1:numel(channels)
                for chi=channels_order
                    data_aux_p=get_responses_channel_align(grapes.rasters.(channels{chi}), channels(chi),...
                                                                 selected_stim, unit_label, ifr_calclator, ...
                                                                 resp_conf, rank_config, min_ifr_thr_list(chi));
                    data = [data;data_aux_p];
                end
        ...
        data=get_responses_channel_align(grapes.rasters.(channels{1}), channels(1), ...
                                   selected_stim, unit_label, ifr_calclator, ...
...
% if numel(channels)>1
%         for chi = 1:numel(channels)
%             futures(chi) = parfeval(@get_responses_channel_align,1,grapes.rasters.(channels{chi}),channels(chi),selected_stim,unit_label,ifr_calclator,resp_conf,rank_config);
%         end
%         for i=1:numel(channels)
%             wait(futures(i));  
%             data_aux_p = fetchOutputs(futures(i));
...
%         end
%     else
%         data=get_responses_channel_align(grapes.rasters.(channels{1}),channels(1),selected_stim,unit_label,ifr_calclator,resp_conf,rank_config);
%     end
end

function data=get_responses_channel_align(chrasters, chlabel, selected_stim, ...
    ...
    for cli = 1:numel(classes)
        ...
        sp_spread_post = NaN*ones(lstim,max(nrows));
        sp_align_post = NaN*ones(lstim,max(nrows));
            ...
            for ist =1:lstim
                ...
                if iscell(spikes1)
                    for jj=1:numel(spikes1)
                        ...
                        sp_align_post(ist,jj)=median(spikes1{jj}((spikes1{jj}< resp_conf.tmax_median) & (spikes1{jj}> resp_conf.tmin_median)));
                        sp_spread_post(ist,jj)=std(spikes1{jj}((spikes1{jj}< resp_conf.tmax_median) & (spikes1{jj}> resp_conf.tmin_median)),'omitnan');
                    end  
                else
        ...
        %             tons(ist) = onset(ist);
                ...
                if iscell(spikes1)
                    for jj=1:numel(spikes1)
                        ...
                        sp_align_post(ist,jj)=median(spikes1{jj}((spikes1{jj}< tons(ist)+rank_config.win_length) & (spikes1{jj}> tons(ist))));
                        sp_spread_post(ist,jj)=std(spikes1{jj}((spikes1{jj}< tons(ist)+rank_config.win_length) & (spikes1{jj}> tons(ist))));
                    end
                else
                    for jj=1:size(spikes1,1)
                        sp_count_base(ist,jj)=sum((spikes1(jj,:)< -resp_conf.tmin_median) & (spikes1(jj,:)> -resp_conf.tmax_median));
...
%         std_base = std(medians_base);   
        ...
        sum_base = sum(sp_count_base, 2, 'omitnan');
        sum_post = sum(sp_count_post, 2, 'omitnan');
        zscore = (median_post - mu_base_med)/(std_base+1e-10); % add small value to avoid division by zero

        penalty_strength = 50.0;  
        
        [n_rows, n_cols] = size(sp_spread_post);
        spread_score = sum(sp_spread_post, 2, 'omitnan');
        
        % nan_counts = sum(isnan(sp_align_post), 2);
        % penalty_ratio = nan_counts ./ n_cols;
        % spread_score = spread_score + (penalty_strength .* penalty_ratio);

        [n_rows, n_cols] = size(sp_align_post);
        align_score = std(sp_align_post, 0, 2, 'omitnan');
        
        nan_counts = sum(isnan(sp_align_post), 2);
        penalty_ratio = nan_counts ./ n_cols;
        align_score = align_score + (penalty_strength .* penalty_ratio);
        align_score = align_score - sum_post + sum_base;      
        ...
        data_u = table(ntrials,IFR,onset,tons,dura,good_lat,zscore,median_post,mu_b,s_b,p_value_sign,min_spk_test,p_test, align_score);
