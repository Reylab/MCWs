function grapes = update_grapes(grapes,pics_onset,stimulus,sp_index,channels, ...
                                chan_labels,domu,classes,stim_numbers,scri, is_online)
...
if ~exist('is_online','var') || isempty(stim_numbers)
    is_online = false;
end
sr = 30000;
...
for chi=1:length(channels)
    ...
    if domu
        if isfield(grapes.rasters.(chanN).details,'mu') && is_online
    ...
    if ~domu && ~isempty(classes{chi})
        ...
        for ci=1:numel(ch_cls)
            ...
            if isfield(grapes.rasters.(chanN).details, clname) && is_online
