classdef ncs_wc_reader < handle
	...
	methods 
        function obj = ncs_wc_reader(par, raw_filename)
            ...
            timestamp2sec = 512/(128*1e9);
            obj.sr = 512/(obj.dt*timestamp2sec);            % sampling rate (in Hz).
            time0 = TimeStamps(1); 
            
            tmin = time0 + par.tmin/timestamp2sec; %min time to read (in micro-sec)
            ...
            if strcmp(par.tmax,'all')
                ...
                tmax = time0 + par.tmax/timestamp2sec;                   %max time to read (in micro-sec)  
            ...
            lts = (index_tfinal - index_tinitial)*512;
            %Segments the data in par.segments pieces
            obj.max_segments = ceil((tmax - tmin)*timestamp2sec/ ...
                    (par.segments_length * 60));         %number of segments in which data is cutted
            segmentLength = floor((lts/512)/obj.max_segments);
            ...
            header = textread(obj.raw_filename,'%s',25,'commentstyle','shell');

			obj.num_scale_factor = 1e4 * str2num(header{find(strcmp('-ADBitVolts', header))+1});
			%for older versions:
