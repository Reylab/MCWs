...
try
    ...
    ISI_max = eval(['par.nbins' num2str(axes_nr-1) ]);
    xlim(isi_ax, [0 ISI_max]);
    hold(isi_ax,'on');
    for i_freq=1:floor(ISI_max / (1000/par.line_freq))
        interval = i_freq*(1000/par.line_freq);
        line(isi_ax,[interval,interval],ylim(isi_ax),'LineWidth',0.8,'LineStyle',':','Color','r')
    end
    hold(isi_ax,'off');
