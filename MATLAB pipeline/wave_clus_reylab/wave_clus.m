...
elseif data_handler.with_results
    ...
    if isempty(temp)
        message = sprintf('Unable to sort, just %d spikes in file.',size(spikes,1));
        msgbox(message,'Wave_clus error')
        return
    end
...
plot_spikes_wc(handles); %This function edits userdata
USER_DATA = get(handles.wave_clus_figure,'userdata');
set(handles.wave_clus_figure,'userdata',USER_DATA);
...
set(handles.wave_clus_figure,'userdata',USER_DATA)
plot_spikes_wc(handles) % plot_spikes updates USER_DATA{11}
