...
time_taken = toc(spike_det_start);
time_taken_mins = floor(time_taken / 60);
time_taken_secs = mod(time_taken, 60);
fprintf('%d ''spikes'' files done in %d mins %0.0f seconds.\n', ...
        length(filenames), time_taken_mins, time_taken_secs)
...
%     par.freq_priority = [30000, 2000, 10000, 1000, 500]; %in decresing priority order
