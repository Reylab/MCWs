function Plot_collisions_bundle(channels,notchfilter,filt_order,detect,tmin)
...
if ~exist('micros', 'var'), micros = true; end
...
% par_macro.detect_fmin = 300;
% par_macro.detect_fmax = 999;
par_macro.auto = 1;

par_micro.sr = 30000;
% par_micro.detect_fmin = 1;
% par_micro.detect_fmax = 120;
par_micro.detect_fmin = 300;
par_micro.detect_fmax = 3000;
% par_micro.detect_fmax = 999;
par_micro.auto = 0;
...
% ----------
% NOISE TEST
% bundles_to_plot = {'bundle 1';'bundle 2';'bundle 3';'bundle 4';'bundle 5';'bundle 6';'bundle 7';'bundle 8'};
% bundles_to_plot = {'m-bundle 1'};
% ----------
...
fig_num = 1032;
figure(fig_num)
set(fig_num, 'PaperUnits', 'inches', 'PaperType', 'A4', 'PaperPositionMode', 'auto','units','normalized','outerposition',[0 0 1 1], 'Visible', 'on') 

% ----------
% NOISE TEST
% chans = [1:8;9:16;17:24;25:32;33:40;41:48;49:56;57:64];
% chans = [1:8;9:16;17:24;25:32];
% chans = [1:8];
% ----------
...
for ibun = 1:length(bundles_to_plot)
    ...
    load(sprintf('%s_collisions.mat', NSx(posch).bundle));
    ...
    sample_count = par.sr * rec_length;
    tot_secs = NSx(posch).lts / par.sr;

    for tmin = 10: rec_length: tot_secs - rec_length
        ...
        collisions = collision_times(collision_times > min_record & collision_times < max_record);
        if numel(collisions) == 0
            fprintf('No collisions between %d and %d', min_record, max_record);
            continue
        end
        ...
        %     nspk=nnz(diff(xaux)>ref)+1;
            ...
            y_collisions = ones(size(collisions)) * (thr+30);
            scatter(collisions/par.sr,y_collisions,'r*')
    ...
    %                         ylim([-2*Vlim Vlim])
                ...
                if notchfilter==0  || nnz(arrayfun(@(x) (x.chID==channel1),process_info))==0
                    text((tmax-tmin)/2+tmin+5,for_text+max(ylim),sprintf('%s. %d spikes. thr = %2.2f',NSx(posch).label,nspk,thr),'fontsize',10)
                ...
                channel_text_macro{cont} = sprintf('%s. %d and %d notches applied below and above %d Hz',NSx(posch).label,cant_notches1l,cant_notches1h,200);
                    ...
                    factor_thr=5;
                    thr = factor_thr * median(abs(xd))/0.6745;
                    channel_text_macro{cont} = sprintf('%s. thr = %2.2f',NSx(posch).label,thr);
                end
            end 
            clear xd;         
    ...
    %                     end 
                ...
                title_out = sprintf('%s.   bundle  %s.   fmin %d Hz. fmax %d Hz. Collisions: %d',folderName,bundle,par.detect_fmin,par.detect_fmax, numel(collisions));
