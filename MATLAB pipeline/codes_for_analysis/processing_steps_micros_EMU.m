...
elseif contains(name,'DESKTOP-DQR054O'), dir_base = 'C:\Users\EMU - Behavior\Documents\MATLAB\Behavioral Tasks\HR\codes_emu';  % EMU BEH PC  
end
...
if CUT_SEGMENTS
    if extract_events && strcmp(which_system_micro,'RIP')
        [ns_status, hFile] = ns_OpenFile(['./' filename_NEV], 'single');
        entity_ix = find(strcmp({hFile.Entity.Reason},'Parallel Input'));
        nevents = hFile.Entity(entity_ix).Count;
        events = zeros(nevents,1); %in seconds
        val = zeros(nevents,1);
        for ei = 1:nevents
            [~, events(ei), val(ei), ~] = ns_GetEventData(hFile,1,ei);
        end
        load('experiment_properties.mat', 'experiment')

        nsegments = experiment.Nseq; %this wont work if the experiment was aborted
        lines_onoff_value = experiment.lines_onoff;
        line_onoff = val==lines_onoff_value;
        if sum(line_onoff) ~= (nsegments*2)
            error('Found %d line_onoff events, should be %d.',sum(line_onoff)  ,2*nsegments)
        end

        times = events(line_onoff);
        inits = times(1:2:end);
        ends = times(2:2:end);
        concat_nsx_segments('.',[inits,ends],'segmented');
        copyfile('experiment_properties.mat',['segmented' filesep 'experiment_properties.mat'])
        copyfile('stimulus.mat',['segmented' filesep 'stimulus.mat'])

        cd('segmented')
    else
        error('cut segments only for ripple with daq events')
    end
end
