...
class MapfileGenerator(QWidget):
    ...
    def initUI(self):
        ...
        self.group_box1.setGeometry(270, 10, 210, 640)

        self.generated_channels_text = QTextEdit(self)
        self.generated_channels_text.setGeometry(280, 45, 190, 592)
    ...
    def generate_mapfile(self):
        ...
        for port in range(1, ports + 1):
            ...
            for channel in range(1, channels_per_port + 1):
                ...
                generated_channels_for_entry.append(line)  # Agrega el canal a la lista
            self.generated_content[entry].append(port_content)

        self.generated_channels.extend(generated_channels_for_entry)  # Extiende la lista de canales generados
        self.update_generated_channels_text()
        self.num_ports[entry] = ports
        message = f"{port} leads were generated with {channels_per_port} channels each for port {entry}."
