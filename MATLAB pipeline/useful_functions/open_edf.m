function [hdr, record] = open_edf(fname, varargin)
...
%         structure record.
%
% [...] = edfread(fname, 'asint16', asint16_bool)
%         If asint16_bool is true, output will be the int16 raw data
%
...
%
% Modifications:
% 22/9/22 Added the asint16 input argument
%
...
if nargout > 1 || assignToVariables
    ...
    for recnum = 1:hdr.records
        for ii = 1:hdr.ns
            ...
            if ismember(ii,targetSignals)
                ...
                if asint16
                    tmpdata(recnum).data{ii} = fread(fid,hdr.samples(ii),'int16=>int16');
                else
    ...
    if asint16
        record = zeros(numel(hdr.label), hdr.samples(1)*hdr.records,'int16');
    else
