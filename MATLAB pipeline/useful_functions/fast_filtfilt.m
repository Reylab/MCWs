function y = fast_filtfilt(b,a,x)
%copied and edited from matlab function
    ...
    %#codegen
    if ~exist('FilterX_fc','file')
        y = filtfilt(double(b),double(a),double(x));
        return
    end
    narginchk(3,3);
    
    coder.internal.assert(isa(b,'double') && isa(a,'double'),'signal:filtfilt:NotSupported');
...
function y = ffOneChan(b,a,xc,zi,nfact,L)
    ...
    for ii=1:L
        ...
        [~,zo] = FilterX_fc(b(:,ii),a(:,ii), xt(:,1), zi(:,ii)*xt(1)); % yc1 not needed
        [yc2,zo] = FilterX_fc(b(:,ii),a(:,ii), xc(:,1), zo);
        xt = -xc(end-1:-1:end-nfact(1,1),1) + 2*xc(end,1);
        yc3 = FilterX_fc(b(:,ii),a(:,ii), xt(:,1), zo);
        ...
        [~,zo] = FilterX_fc(b(:,ii),a(:,ii), yc3(:,1), zi(:,ii)*yc3(end,1),'reverse');
        yc5 = FilterX_fc(b(:,ii),a(:,ii), yc2(:,1), zo,'reverse');
...
function y = ffMultiChan(b,a,xc,zi,nfact,L)
    ...
    for ii=1:L
        ...
        [~,zo] = FilterX_fc(b(:,ii),a(:,ii),xt,zi(:,ii)*xt(1,:)); % outer product
        [yc2,zo] = FilterX_fc(b(:,ii),a(:,ii),xc,zo);
        xt = bsxfun(@minus, 2*xc(end,:), xc(end-1:-1:end-nfact(1,1),:));
        yc3 = FilterX_fc(b(:,ii),a(:,ii),xt,zo);
        
        [~,zo] = FilterX_fc(b(:,ii),a(:,ii),yc3,zi(:,ii)*yc3(end,:),'reverse'); % outer product
        yc5 = FilterX_fc(b(:,ii),a(:,ii),yc2, zo,'reverse');
