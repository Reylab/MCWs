...
def get_time_hrs_mins_secs_ms(seconds):
    """
    Get time in hours, minutes, seconds, and milliseconds.
    
    Returns:
        tuple: Current time in hours, minutes, seconds, and milliseconds
    """
    hrs, secs = divmod(seconds, 3600)
    mins, secs = divmod(secs, 60)
    secs, ms = divmod(secs*1000, 1000)
...
def get_elapsed_time(start_time=None):
    ...
    Returns:
        tuple: Elapsed time in hours, minutes, and seconds
    """
    elapsed_time = time.time() - start_time
    
    return get_time_hrs_mins_secs_ms(elapsed_time)
...
def MCW_location(location):
    ...
    if os.name == 'nt':
        ...
        xippmexix = next((i for i, valname in enumerate(valnames) if re.search('(Ripple\\\\Trellis\\\\Tools\\\\xippmex\\\\)$', valname)), None)
        if xippmexix is not None:
