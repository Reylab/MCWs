# Modified version of https://stackoverflow.com/questions/62363953/how-to-create-toggle-switch-button-in-qt-designer
from PyQt5.QtCore import QObject, QSize, QPointF, QPropertyAnimation, QEasingCurve, pyqtProperty, pyqtSlot, Qt
from PyQt5.QtGui import  QPainter, QPalette, QLinearGradient, QGradient, QFont
...
class SwitchPrivate(QObject):
    ...
    def draw(self, painter):
        ...
        if self.mPointer.isChecked():  # If the switch is on
            # Draw the background green
            self.mGradient.setColorAt(0, Qt.darkGreen)
            self.mGradient.setColorAt(1, Qt.green)
            ...
            # painter.setFont(QFont('Arial', r.height()//2, QFont.Bold))
            # painter.drawText(r.adjusted(margin*4, margin, -r.width()//2, 0), Qt.AlignRight, 'EDIT')
            ...
            # painter.setPen(button)
            # painter.setFont(QFont('Arial', r.height()//2, QFont.Bold))
            # painter.drawText(r.adjusted(r.width()//2, 0, 0, 0), Qt.AlignCenter, 'VIEW')
...
class Switch(QAbstractButton):
    def __init__(self, parent=None):
        ...
        self.toggled.connect(self.dPtr.animate)
