classdef readInData < handle
	...
	methods 
        ...
        function [spikes, index_ts,spikes_all,index_all_ts] = load_spikes_withCollisions(obj)
            if ~ (obj.with_spikes || obj.with_wc_spikes)
                ME = MException('MyComponent:noSpikesFound', 'Wave_Clus couldn''t find a file with spikes');
                throw(ME)
            end
...
%                 load(spikes_file,'spikes','index');
                ...
                if ~ exist('index_ts','var')                    %for retrocompatibility
                    index_ts = index;
                    index_all_ts = index_all;
                end
            else
%                 [spikes, index_ts] = obj.file_reader.load_spikes();
                [spikes, index_ts,spikes_all,index_all_ts] = obj.file_reader.load_spikes_withCollisions();
            end
