...
% faster. With the fast Mex function FilterX_fc, FiltFiltM is 80% to 95% faster
...
% - The results for signals of type SINGLE are more accurate, because FilterX_fc
...
%   FilterX_fc.c can be compiled: Call FilterM() without inputs. See there for
...
persistent hasFilterX_fc
if isempty(hasFilterX_fc)
   hasFilterX_fc = ~isempty(which('FilterX_fc'));
   if ~hasFilterX_fc && ~isempty(which('FilterM'));
      fprintf(['::: %s: Matlab''s FILTER is used.\n', ...
         '    Run FilterM without inputs to compile FilterX_fc.c\n'], ...
         mfilename);
   end
end
...
if hasFilterX_fc  % Use the faster C-mex filter function: -------------------------
   % Filter initial reflected signal:
   [dum, Zi] = FilterX_fc(b, a, Xi, IC * Xi(1, :));   %#ok<ASGLU>
   
   % Use the final conditions of the initial part for the actual signal:
   [Ys, Zs]  = FilterX_fc(b, a, X,  Zi);              % "s"teady state
   Yf        = FilterX_fc(b, a, Xf, Zs);              % "f"inal conditions
   
   % Filter signal again in reverse order:
   [dum, Zf] = FilterX_fc(b, a, Yf, IC * Yf(nEdge, :), 'reverse');  %#ok<ASGLU>
   Y         = FilterX_fc(b, a, Ys, Zf, 'reverse');
