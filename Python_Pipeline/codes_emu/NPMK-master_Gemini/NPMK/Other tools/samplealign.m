...
for i=1:nargin
    ...
    if ~isempty(segmentinds{i})
        ...
        durations{i} = segmentedts{i}(end) - segmentedts{i}(1); % duration in nanoseconds (TimeRes)
...
for j=1:nargin
    ...
    if length(samplingrates{j}) > 1
        ...
        if addedsamples > 0 % what if undersampling + packets lost? then we need to fill a gap (with zeros?) not equally spaces throughout the recording 
            ...
            while startind < datalengths{j}
                ...
                startind = startind + gap; %repeats a sample after gap samples?
            end
        elseif addedsamples < 0
