...
function processing_steps_MCW(varargin)
...
if par.extract_events && ~par.fast_analysis && par.micros
    nev_processing_start = tic;
    fprintf('NEV processing BEGIN..\n')
   ...
   % if contains(exp_type,'RSVP_online')
    if contains(exp_type,'RSVP')
        if USE_PHOTODIODE
    ...
    time_taken = toc(nev_processing_start);
    time_taken_mins = floor(time_taken / 60);
    time_taken_secs = mod(time_taken, 60);
    fprintf('NEV processing DONE in %d mins %0.0f seconds.\n', time_taken_mins, time_taken_secs)
end
%% spike detection
if par.micros
    ...
    %% collision within probes
    disp('separate_collisions BEGIN..')
    separate_collisions(channels)

    %% Separate task spikes
    if ~par.fast_analysis
        disp('separate task spikes BEGIN..')
        separate_task_spikes(channels, par.use_blanks)
    end
    ...
    if par.do_loop_plot  && ~par.fast_analysis
        ...
        if strcmp(exp_type,'RSVPSCR')
            ...
            do_structure_mu_BCM_online3(channels,exp_type, par.use_blanks, par.circshiftblanks)
    ...
    %%
        ...
        if par.do_loop_plot  && ~par.fast_analysis
            ...
            if strcmp(exp_type,'RSVPSCR')
                do_structure_sorted_BCM_online3(clustered_channels, par.use_blanks, par.circshiftblanks)
...
%             elseif contains(exp_type,'RSVP_online')
    ...
    if ~par.fast_analysis
        ...
        plot_grapes_as_online('grapes_offline',true,'channels2plot', 'all', 'stim_list', 'all', 'order_by_rank', true, ...
                              ...
                              'ch_grapes_nwins', 2, 'extra_lbl', '', 'use_blanks', par.use_blanks, ...
...
%     channels_redo = 322;
%     do_structure_sorted_BCM_online3(channels_redo)
%     plot_grapes_as_online(1,channels_redo)
% end
...
custompath.rm
time_taken = toc(processing_steps_start);
time_taken_mins = floor(time_taken / 60);
time_taken_secs = mod(time_taken, 60);
fprintf('processing_steps_MCW done in %d mins %0.0f seconds.\n', time_taken_mins, time_taken_secs)