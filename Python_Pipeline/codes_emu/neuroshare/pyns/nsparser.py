...
class NevParser:
    ...
    def get_basic_header(self):
        ...
        try: 
            ...
            # decode the first item in tuple
            tup = (tup[0].decode('utf-8'),) + tup[1:]
        except:
            raise NeuroshareError(NSReturnTypes.NS_BADFILE,
                                  "failed reading file")
        if tup[0] != "NEURALEV":
    ...
    def get_data_packet(self, packet_index=None):
        ...
        waveform = np.array(packet_tup[4:], dtype=np.int16)
...
class Nsx21Parser:
    ...
    def get_basic_header(self):
        ...
        try:
            ...
            raise NeuroshareError(NSReturnTypes.NS_BADFILE,
                                  "failed reading file")
        channel_ids = np.array(header_tup[4:])
    ...
    def get_analog_data(self, channel, start_index, index_count): 
        ...
        waveform = np.zeros(index_count, dtype=np.double)
        bin_count = 0
        ...
        waveform = np.resize(waveform, bin_count)
        # return pointer?
        # self.fid.seek(0, os.SEEK_SET)
        return waveform        
...
#        self.scale = self.scale / (header.max_dig_value - header.min_dig_value)
    ...
    def get_analog_packet(self, channel_index, start_index, index_count): 
        ...
        for data_packet in self.data_packet_list:
            ...
            if remaining_skipped_points <= data_packet[1]:
                self.fid.seek(remaining_skipped_points*self.channel_count*self.bytes_per_point, os.SEEK_CUR)
                break
            skipped_packets += 1
    ...
    def get_analog_data(self, channel_index, start_index, index_count):
        ...
        Parameters:
            ...
            index - how many bins of the waveform to return
        Returns:
            Requested waveform as a np.array
        """
        if index_count <= 0 or \
    ...
    def get_analog_data_full(self, start_index, index_count):
        """Return the analog waveform for Nsx2.2 files.  Returns data 
        starting at the start_index bin and the next index_count bins. If the end 
        of the file is reached before index_count return a waveform with 
        as many bins as are found.   
        Parameters:
            start_index - first bin of electrode data to return
            index_count - how many bins of the waveform to return
        Returns:
            Requested waveform as a np.array
        """
        if index_count <= 0 or \
        ...
        data_arr = self.get_analog_packet_full(start_index, index_count)

        return data_arr

    def get_analog_packet_full(self, start_index, index_count): 
        """Generator to to read a file in large chunks. 
        Parameters:
            channel_index - if specified, only return data for the wanted.  If 
                unspecified, returns all the data for a single time-slice
            packet_index - return the wanted packet.  If unspecified, returns
        ...
        for data_packet in self.data_packet_list:
            self.fid.seek(9, os.SEEK_CUR)
            if remaining_skipped_points <= data_packet[1]:
                self.fid.seek(remaining_skipped_points*self.channel_count*self.bytes_per_point, os.SEEK_CUR) # SM 08/15/24: changed from 2 to self.bytes_per_point
                break
            skipped_packets += 1
            remaining_skipped_points -= data_packet[1]
            self.fid.seek(data_packet[1], os.SEEK_CUR)
        ...
        # Data array to hold data for all channels
        data_arr = np.zeros((self.channel_count, index_count), dtype=datatype)
        while points_read < index_count:
            # Loop over all the pause points as each comes with a header
            # hold the number of data points in each pause
            for ipacket, data_packet in enumerate(self.data_packet_list[skipped_packets:]):
                ...
                while remaining_points > 0 and points_read < index_count:
                    # read 1024 points or the end of the section
                    read_points = min(max_read_points, index_count - points_read)
                    ...
                    #     # raise NeuroshareError(msg)
                    #     read_points = len(read_buffer) / self.bytes_per_point / self.channel_count
                    # use numpy to read the buffer
                    
                    # data = np.frombuffer(read_buffer, dtype=datatype).copy()
                    data = np.fromfile(self.fid, dtype=datatype, count=read_points*self.channel_count).reshape(-1, self.channel_count)

                    # if self.timestamp_resolution == 30000:
                    #     pak_lost = data == np.iinfo(np.int16).min
                    #     data[pak_lost] = 0
                    # else:
                    #     pak_lost = data == np.finfo(np.float32).max
                    #     data[pak_lost] = 0

                    # data = data.reshape(-1, self.channel_count)
                    # add to data array
                    data_arr[:, points_read:points_read+read_points] = data.T
                    # update the remaining points 
                    remaining_points -= read_points
    ...
    def get_data_packet(self, packet_index=None):
        ...
        #data_points = np.array(tup[3:], dtype=np.int16)
