...
for i = 1:nFiles
    ...
    elseif strcmp(fileType, 'NEURALCD') || ...
        ...
        hFile.FileInfo(i).BytesHeaders = fread(fid, 1, '*uint32');
        % deblank_tic = tic;
        label = char(fread(fid, 16)');
        label = strtrim(label);
        hFile.FileInfo(i).Label = label;
        % hFile.FileInfo(i).Label = deblank(fread(fid, 16, '*char')');
        % time_taken = toc(deblank_tic);
        % time_taken_mins = floor(time_taken / 60);
        % time_taken_secs = mod(time_taken, 60);
        % fprintf('deblank done in %d mins %0.0f seconds.\n', time_taken_mins, time_taken_secs)
        % skip: Comment
        fseek(fid, 256, 0);
        hFile.FileInfo(i).Period = fread(fid, 1, 'uint32');
        ...
        for j = EC+1:EC + chanCount
            ...
            hFile.Entity(j).Units = char(fread(fid, 16)');
